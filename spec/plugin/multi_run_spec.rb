...
describe "multi_run plugin" do 
  it "adds Roda.run method for setting up prefix delegations to other rack apps" do
    ...
    app.run "a", Class.new(Roda).class_exec{route{|_| "a1"}; app}
    ...
    app.run "b", Class.new(Roda).class_exec{route{|_| "b1"}; app}
    ...
    app.run "b/a", Class.new(Roda).class_exec{route{|_| "b2"}; app}
  ...
  it "works when freezing the app" do
    ...
    proc{app.run "a", Class.new(Roda).class_exec{route{|_| "a1"}; app}}.must_raise
  end

  it "supports multi_run_apps" do
    app(:multi_run){|r|}
    app.multi_run_apps.must_equal({})
    a = Class.new(Roda).class_exec{route{|_| "a1"}; app}
    app.run :a, a
  ...
  it "works when subclassing" do
    ...
    a.run "b", Class.new(Roda).class_exec{route{|_| "b2"}; app}
    app.run "b", Class.new(Roda).class_exec{route{|_| "b1"}; app}
...
describe "multi_run plugin with blocks for Roda.run" do 
  it "adds Roda.run method for setting up prefix delegations to other rack apps" do
    ...
    loaded = []
    app.run("a"){loaded << :a1; Class.new(Roda).class_exec{route{|_| "a1"}; app}}
    ...
    app.run("b"){loaded << :b1; Class.new(Roda).class_exec{route{|_| "b1"}; app}}
    ...
    app.run("b/a"){loaded << :b2; Class.new(Roda).class_exec{route{|_| "b2"}; app}}
  ...
  it "registering app removes app block and vice versa" do
    ...
    app.run "a", Class.new(Roda).class_exec{route{|_| "a1"}; app}
    body("/a").must_equal 'a1'
    app.run("a"){Class.new(Roda).class_exec{route{|_| "a2"}; app}}
    body("/a").must_equal 'a2'
    app.run "a", Class.new(Roda).class_exec{route{|_| "a3"}; app}
  ...
  it "supports both apps and app blocks" do
    ...
    app.run "a", Class.new(Roda).class_exec{route{|_| "a"}; app}
    app.run("b"){Class.new(Roda).class_exec{route{|_| "b"}; app}}
    body("/a").must_equal 'a'
    body("/b").must_equal 'b'
  end
end
