...
Roda is designed with the following SUPER goals in mind:
...
Roda is designed to avoid hidden control flow, so it is easy to
follow the control flow you write in your routing tree to see
the application's entire handling process for a given request.
...
Sets match if the next segment in the request path matches one of the
elements in the set (non-String elements in the set are ignored). The matched
element is yielded.

  Set['page1', 'page2'] # matches "/page1", "/page2"
  Set[]                 # does not match anything

In general, a Set matcher has the same behavior as an Array matcher 
containing only the set's string elements, but will perform better for
when there are a large number of elements in the set.
...
:check_arity :: Whether arity for blocks passed to Roda should be checked
                ...
                or need to be wrapped. For backwards compatibility, this can be
                set to +true+, so Roda will check blocks and handle cases where
                the arity of the block does not match the expected arity. The default 
                is +:warn+, which issues warnings whenever Roda detects an
                ...
                by default. In Roda 4, this option will be removed, and +false+
                will be the only behavior.
:check_dynamic_arity :: Similar to :check_arity, but used for checking blocks
                        ...
                        In Roda 4, Roda will not check dynamic arity, and arity
                        mismatches will result in exceptions.
...
application level using the +default_headers+ plugin:
...
Roda does not ship with integrated support for code reloading, but there are
existing reloaders that will work with Roda apps:

{Zeitwerk}[https://github.com/fxn/zeitwerk] is the most popular Ruby code
reloading library.  It requires minimal setup and handles most cases. It
overrides +require+ when activated. It supports lazily loading code
(autoloading). It requires some file path conventions, and requires each file
define a single top-level constant. It doesn't fully support explicit requires
of dependencies.  Zeitwerk is probably the best choice if you prefer your code
to be automatically loaded and agree with the Zeitwerk code organization
conventions.

{rack-unreloader}[https://github.com/jeremyevans/rack-unreloader] implements a
Rack middleware that detects file changes and reloads only the files that
change.  It wraps a Rack app and records loaded constants per file so that it
knows which constants to unload when reloading a file.  It doesn't override
+require+ or apply any monkey patches, instead using its own API to require
files and specify dependencies. The main advantage over other reloaders is that
it only unloads constants that are required by the file that was modified (a
much faster approach), it supports splitting classes into multiple files (common
for Roda apps using the hash_branches plugin), and it does not require any file
path conventions. By default, it unloads newly detected constants, which can be
dangerous.  Users are advised to review the documentation before using it.  It
supports both eager loading (require) and lazy loading (autoloading).
rack-unreloader is probably the best choice if performance is important, you
have classes split into multiple files, or you would like to avoid monkey
patching +Kernel.require+.

{auto_reloader}[https://github.com/rosenfeld/auto_reloader] provides transparent
reloading for all files reached from one of the +reloadable_paths+ option
entries, by detecting new top-level constants and removing them when any of the
reloadable loaded files changes. It overrides +require+ and +require_relative+
when activated. No configurations other than +reloadable_paths+ are required.
Autoloading isn't supported by design.  auto_reloader may be the best choice if
you do not want to use Zeitwerk's conventions and are OK with monkey patching.

{rerun}[https://github.com/alexch/rerun] uses a fork/exec approach for loading
new versions of your app. It works without any changes to application code, but
may be slower as it has to reload the entire application on every change. rerun
is probably the best choice if you value simplicity over performance for code
reloading.

There is no one reloading solution that is the best for all applications and
development approaches.  Consider your needs and the tradeoffs of each of the
reloading approaches, and pick the one you think will work best.  If you are
unsure where to start, it may be best to start with Zeitwerk, and only consider
...
required to run the current version of Roda is 2.0, and the minimum JRuby version is
9.0.0.0.
