...
Roda does not ship with integrated support for code reloading, but there are many good
options available:

{Zeitwerk}[https://github.com/fxn/zeitwerk] is the most popular Ruby code reloading library.
It requires minimal setup and handles most cases. It overrides +require+ when
activated. It supports loading code on demand (autoloading), while enforcing some file path
conventions. Each file should define a single top-level constant. On the other hand,
explicitly requiring dependencies with Zeitwerk is currently considered
{experimental and undocumented}[https://github.com/fxn/zeitwerk/blob/main/lib/zeitwerk/core_ext/kernel.rb#L16].
Use of +require_relative+ {isn't currently supported}[https://github.com/fxn/zeitwerk/blob/main/lib/zeitwerk/core_ext/kernel.rb#L45].
It's battle tested by Rails users. Pick Zeitwerk if you prefer your code to be automatically
loaded and agree with the Zeitwerk convention rules to organize your code.

{rack-unreloader}[https://github.com/jeremyevans/rack-unreloader] implements a Rack
middleware that detects changes upon web requests. It wraps a Rack app and records all
loaded constants so that it knows what to unload in the next request on code updates.
It doesn't override +require+ or apply any monkey patches. It unloads all newly detected
constants, including standard library constants like JSON, which can be dangerous. Users are
advised to load core classes before accepting requests (see the documentation). You can
optionally enable autoloading as long as you provide the rules to translate a file path to
the constant name. By default autoloading is disabled and the dependencies must be explicitly
required. Pick rack-unreloader if you'd rather avoid monkey patching +Kernel.require+ and prefer
to modify the application code to work with this tool instead.

{auto_reloader}[https://github.com/rosenfeld/auto_reloader] provides transparent reloading for
...
configurations other than reloadable_paths are required. It detects code changes by watching
files if the +listen+ gem is installed or by checking if any of the reloadable files have been
changed upon each request otherwise. Autoloading isn't supported by design. Pick auto_reloader
if you'd prefer explicit dependency management, simplicity and plain unmodified Ruby in
production and don't care about monkey-patching require and require_relative in the development
environment.

{rerun}[https://github.com/alexch/rerun] uses a fork/exec approach for loading new versions of
your app. It works without any changes to application code, but may be slower as it has to
reload the entire application on every change. It doesn't support autoloading code. Pick rerun
if you value simplicity over performance in the development environment and prefer explicit
